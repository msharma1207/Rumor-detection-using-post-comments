...
for i in range(fakes):
    label.append(0)  #432 fakes , then 624 real
for i in range(real):
    label.append(1)
...
######################################
with open('unencoded_train_cmt_pl','wb') as f3: pickle.dump(train_cmt, f3)
with open('unencoded_test_cmt_pl','wb') as f4: pickle.dump(test_cmt, f4)

label = np.array(label)
with open('label_pl_final','wb') as f2: pickle.dump(label, f2)  # DO keras.to_catogorical after reading

encoded_train_x = _encode_post(train_post)
trpost_reshaped1 = encoded_train_x.reshape(encoded_train_x.shape[0], -1)

with open('encoded_train_post_pl','wb') as f: pickle.dump(trpost_reshaped1, f)

encoded_val_x = _encode_post(test_post)
tepost_reshaped2 = encoded_val_x.reshape(encoded_val_x.shape[0], -1)

with open('encoded_test_post_pl','wb') as f1: pickle.dump(tepost_reshaped2, f1)
